...
class TradingEnv(gym.Env):
    ...
    def __init__(
        ...
        # CASE 2: Real stock loading
            ...
            csvName = "".join(
                ["Data/", marketSymbol, "_", startingDate, "_", endingDate]
            )
            ...
            # Otherwise, download the stock market data from Yahoo Finance and save it in the database
                ...
                try:
                    self.data = downloader1.getDailyData(
                        marketSymbol, startingDate, endingDate
                    )
                except:
                    self.data = downloader2.getDailyData(
                        marketSymbol, startingDate, endingDate
                    )
        ...
        self.data.interpolate(
            method="linear", limit=5, limit_area="inside", inplace=True
        )
        ...
        self.state = [
            self.data["Close"][0:stateLength].tolist(),
    ...
    def computeLowerBound(self, cash, numberOfShares, price):
        ...
        deltaValues = -cash - numberOfShares * price * (1 + self.epsilon) * (
    ...
    def step(self, action):
        ...
        if action == 1:
            ...
            # Case b: No position -> Long
                ...
                self.data["Cash"][t] = self.data["Cash"][
                    t - 1
            ...
            if self.data["Position"][t - 1] == -1:
                ...
                if lowerBound <= 0:
                    ...
                    numberOfSharesToBuy = min(
                        math.floor(lowerBound), self.numberOfShares
                    )
                    self.numberOfShares -= numberOfSharesToBuy
                    self.data["Cash"][t] = self.data["Cash"][
                        t - 1
                    ] - numberOfSharesToBuy * self.data["Close"][t] * (
                        1 + self.transactionCosts
                    )
                    self.data["Holdings"][t] = (
                        -self.numberOfShares * self.data["Close"][t]
        ...
        self.data["Returns"][t] = (
            self.data["Money"][t] - self.data["Money"][t - 1]
        ) / self.data["Money"][t - 1]
        ...
        if not customReward:
            ...
            self.reward = (
                self.data["Close"][t - 1] - self.data["Close"][t]
            ) / self.data["Close"][t - 1]
        ...
        if otherAction == 1:
            ...
            if self.data["Position"][t - 1] == 1:
                ...
                otherCash = self.data["Cash"][t - 1] - numberOfShares * self.data[
                    "Close"
                ][t] * (1 + self.transactionCosts)
                otherHoldings = numberOfShares * self.data["Close"][t]
            else:
                otherCash = self.data["Cash"][t - 1] - numberOfShares * self.data[
                    "Close"
                ...
                otherCash = otherCash - numberOfShares * self.data["Close"][t] * (
                    1 + self.transactionCosts
                )
                otherHoldings = numberOfShares * self.data["Close"][t]
        else:
        ...
        if not customReward:
            otherReward = (otherMoney - self.data["Money"][t - 1]) / self.data["Money"][
                t - 1
            ]
        else:
            otherReward = (
    ...
    def render(self):
        ...
        ax1.plot(
            self.data.loc[self.data["Action"] == 1.0].index,
