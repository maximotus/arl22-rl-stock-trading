...
class PerformanceEstimator:
    ...
    def computeAnnualizedReturn(self):
        ...
        if cumulativeReturn > -1:
            self.annualizedReturn = 100 * (
                ((1 + cumulativeReturn) ** (365 / timeElapsed)) - 1
            )
        else:
            self.annualizedReturn = -100
    ...
    def computeSharpeRatio(self, riskFreeRate=0):
        ...
        if expectedReturn != 0 and volatility != 0:
            self.sharpeRatio = (
                np.sqrt(252) * (expectedReturn - riskFreeRate) / volatility
            )
        else:
            self.sharpeRatio = 0
    ...
    def computeMaxDrawdown(self, plotting=False):
        ...
        if plotting:
            ...
            plt.plot(
                [self.data.iloc[[peak]].index, self.data.iloc[[through]].index],
                [capital[peak], capital[through]],
                "o",
                color="Red",
                markersize=5,
            )
    ...
    def computeProfitability(self):
        ...
        loss = 0
        index = next(
            (i for i in range(len(self.data.index)) if self.data["Action"][i] != 0),
            None,
        )
        if index == None:
    ...
    def computePerformance(self):
        ...
        self.performanceTable = [
            ["Profit & Loss (P&L)", "{0:.0f}".format(self.PnL)],
            ...
            [
                ...
                "{0:.3f}".format(self.averageProfitLossRatio),
            ],
    ...
    def displayPerformance(self, name):
        ...
        tabulation = tabulate(
            self.performanceTable, headers, tablefmt="fancy_grid", stralign="center"
        )
        print(tabulation)
