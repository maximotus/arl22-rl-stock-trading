...
class TradingSimulator:
    ...
    def analyseTimeSeries(
        ...
        # Error message if the stock specified is not valid or not supported
            ...
            print(
                "The stock specified is not valid, only the following stocks are supported:"
            )
    ...
    def plotEntireTrading(self, trainingEnv, testingEnv):
        ...
        ax1.plot(
            data.loc[data["Action"] == 1.0].index,
        ...
        plt.savefig(
            "".join(
                [
                    "Figures/",
                    str(trainingEnv.marketSymbol),
                    "_TrainingTestingRendering",
                    ".png",
                ]
            )
        )
    ...
    def simulateNewStrategy(
        ...
        # Error message if the strategy specified is not valid or not supported
            ...
            print(
                "The strategy specified is not valid, only the following strategies are supported:"
            )
        ...
        trainingEnv = TradingEnv(
            stock, startingDate, splitingDate, money, stateLength, transactionCosts
        )
        ...
        testingEnv = TradingEnv(
            stock, splitingDate, endingDate, money, stateLength, transactionCosts
        )

        # Testing of the trading strategy
        testingEnv = tradingStrategy.testing(
            trainingEnv,
            testingEnv,
            rendering=rendering,
            showPerformance=showPerformance,
    ...
    def simulateExistingStrategy(
        ...
        fileName = "".join(
            ["Strategies/", strategy, "_", stock, "_", startingDate, "_", splitingDate]
        )
        ...
        if exists:
            ...
            raise SystemError(
                "The trading strategy specified does not exist, please provide a valid one."
            )
        ...
        # Testing of the trading strategy
        trainingEnv = tradingStrategy.testing(
            trainingEnv,
            trainingEnv,
            rendering=rendering,
            showPerformance=showPerformance,
    ...
    def evaluateStrategy(
        ...
        tabulation = tabulate(
            performanceTable, headers, tablefmt="fancy_grid", stralign="center"
        )
        print(tabulation)
