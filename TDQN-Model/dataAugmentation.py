...
class DataAugmentation:
    ...
    def streching(self, tradingEnv, factor=1):
        ...
        for i in range(1, len(newTradingEnv.data.index)):
            newTradingEnv.data["Close"][i] = newTradingEnv.data["Close"][i - 1] * (
                1 + returns[i]
            )
            newTradingEnv.data["Low"][i] = (
                newTradingEnv.data["Close"][i]
                * tradingEnv.data["Low"][i]
                / tradingEnv.data["Close"][i]
            )
            newTradingEnv.data["High"][i] = (
                newTradingEnv.data["Close"][i]
                * tradingEnv.data["High"][i]
                / tradingEnv.data["Close"][i]
            )
    ...
    def lowPassFilter(self, tradingEnv, order=5):
        ...
        newTradingEnv.data["Close"] = (
            newTradingEnv.data["Close"].rolling(window=order).mean()
        )
        newTradingEnv.data["Low"] = (
            newTradingEnv.data["Low"].rolling(window=order).mean()
        )
        newTradingEnv.data["High"] = (
            newTradingEnv.data["High"].rolling(window=order).mean()
        )
        newTradingEnv.data["Volume"] = (
            newTradingEnv.data["Volume"].rolling(window=order).mean()
        )
            ...
            for stretch in stretchRange:
                ...
                for order in filterRange:
                    ...
                    for noise in noiseRange:
                        tradingEnvList.append(
                            self.noiseAddition(tradingEnvFiltered, noise)
                        )
